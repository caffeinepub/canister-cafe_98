...
import { useEditAppShowcase } from '../hooks/useQueries';
...
export default function EditAppModal({ showcase, onClose }: EditAppModalProps) {
  const [appName, setAppName] = useState(showcase.appName);
  const [description, setDescription] = useState(showcase.description);
  const [liveLink, setLiveLink] = useState(showcase.liveLink);
  const [category, setCategory] = useState(showcase.category);
  const [hashtagsInput, setHashtagsInput] = useState(showcase.hashtags.join(', '));
  const [thumbnail, setThumbnail] = useState<File | null>(null);
  const [previewUrl, setPreviewUrl] = useState<string | null>(
    showcase.thumbnail?.getDirectURL() || null
  );
  ...
  const editApp = useEditAppShowcase();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else if (keepExistingThumbnail && showcase.thumbnail) {
        thumbnailBlob = showcase.thumbnail;
      }

      const hashtags = parseHashtags(hashtagsInput);

      await editApp.mutateAsync({
        id: showcase.id,
      ...
      const errorMessage = error?.message || 'Failed to update app showcase';
      if (errorMessage.includes('.xyz')) {
        ...
        toast.error('You are not authorized to edit this showcase');
      } else {
  ...
  return (
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit Your App</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-2">
            ...
            <Button type="submit" className="flex-1" disabled={editApp.isPending}>
              {editApp.isPending ? (
                <>
